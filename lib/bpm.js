...
BPM.prototype.taskQuery = function(query) {
    var segment = "", processVariables = [], taskVariables = [], queryHistory;
    if (typeof query === "string") {
        segment =  query;
        query = {};
    } else {
        queryHistory = query['queryHistory'];
        delete query['queryHistory'];
    ...
    return httpjson({
        url: this.bpengine + (queryHistory ? "/history/task/" : "/task/") + segment,
        qs: query || {},
        method: "GET"
    });
};
